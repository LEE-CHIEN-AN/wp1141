...
interface StoreVisitRecordsProps {
  storeId: string
...
const StoreVisitRecords: React.FC<StoreVisitRecordsProps> = ({
  storeId,
  isAuthenticated,
  currentUser,
  onLoginPrompt
  ...
  const renderVisitCard = (visit: VisitRecordWithUser, isMyVisit: boolean = false) => {
    ...
    return (
      <Card key={visit.id} sx={{ mb: 2, borderRadius: 2, boxShadow: 1 }}>
        <CardContent sx={{ p: 2 }}>
            ...
            <Box sx={{ flex: 1 }}>
              ...
              {visit.photos && visit.photos.length > 0 && (
                <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', mt: 1 }}>
                  {visit.photos.slice(0, 3).map((photo: any, index: number) => (
                    <img
                      key={index}
                      src={mediaAPI.getImageUrl(photo.mediaId || photo)}
                      alt={`造訪照片 ${index + 1}`}
                      style={{ 
                        width: 50, 
                        height: 50, 
    ...
    <Box sx={{ 
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        ...
        <Button
          ...
          sx={{
            backgroundColor: isAuthenticated ? '#2F6F4E' : '#E8E0D6',
            color: isAuthenticated ? 'white' : '#999999',
            fontFamily: '"Noto Sans TC", sans-serif',
            '&:hover': { 
              backgroundColor: isAuthenticated ? '#1F4A33' : '#E8E0D6'
            }
          }}
        >
          {isAuthenticated ? '新增紀錄' : '登入後可新增紀錄'}
        </Button>
      </Box>
        ...
        <>
          ...
          {isAuthenticated && myVisits.length > 0 && (
            <Box sx={{ mb: 3 }}>
              ...
              {myVisits.map(visit => renderVisitCard(visit, true))}
            </Box>
          )}
          ...
          <Box>
            ...
            {allVisits.length === 0 ? (
              ...
              <Box>
                {allVisits.map(visit => renderVisitCard(visit))}
              </Box>
            )}
          </Box>
        </>
...
export default StoreVisitRecords
