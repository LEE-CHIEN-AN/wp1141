...
import { createPostMentionNotification } from '@/lib/utils/notifications';
      ...
      if (mentions.length > 0) {
        ...
        if (mentionedUserIds.length > 0) {
          ...
          if (mentionedUserIdsList.length > 0) {
            // 創建 @mention 通知
            await createPostMentionNotification(post.id, uid, mentionedUserIdsList);
          }
        }
      }
    } catch (error) {
      // 忽略 @mention 通知錯誤，不影響貼文創建
