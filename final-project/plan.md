 ...
 **P.S.  測試建議與範圍限制 (Recommended Testing Scope)**
 
地圖載入以及路徑、陰影計算涉及大量運算，請耐心等候結果（團隊已在本地端針對演算法進行優化，惟受限於免費雲端託管環境的速度遠不及本地開發，導致處理時間較長。此次實作也讓我們深刻體會到，此類高密集運算服務確實需要 AWS 等企業級基礎設施的支援。)

由於陰影路徑規劃涉及龐大的幾何運算，且目前後端部署於資源受限的免費雲端環境 (Render Free Tier CPU)，長距離路徑容易因運算逾時 (Timeout) 而導致失敗。

為了獲得最佳的測試體驗，**建議您將測試範圍集中在「台大校園內部」或「公館商圈」周邊的短距離路徑。**
* ✅ **推薦測試：** 公館捷運站 → 台大總圖書館（運算約需 30~60 秒）
* ❌ **避免測試：** 跨行政區長途路線（如：台北車站 → 西門町），極高機率會發生 Timeout。
